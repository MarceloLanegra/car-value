...
describe('AuthService', () => {
  ...
  it('throws an error if signin is called with an unused email', async () => {
    await expect(
      service.signin('asdas@asdas.com', 'asdasdasf'),
  ...
  it('throws an error if an invalid password is provided', async () => {
    ...
    await expect(service.signin('test@gmail.com', 'dasdsa')).rejects.toThrow(
      BadRequestException,
    );
  });

  it('returns a user if correct password is provided', async () => {
    await service.signup('asdf@gmail.com', 'password');

    const user = await service.signin('asdf@gmail.com', 'password');
